/* ============================================================
   ...
   Primitives partagées (flèches, labels, lignes)
...
function buildLineMeasure(p1, p2, labelText, options = {}) {
  ...
  arrowStart.set({ left: p1.x, top: p1.y, angle: angle + 180 });

  const arrowEnd = createOpenArrow(arrowSize, color, strokeWidth);
  arrowEnd.set({ left: p2.x, top: p2.y, angle: angle });
...
async function finalizeScale(p1, p2) {
  ...
  Status.show(`Échelle définie : 1 m = ${State.pixelsPerMeter.toFixed(0)} px`, 'success');
